...
plot_rectangles(unoverlapped_hyperrectangles %>% 
                  mutate(volume_id = new_volume_id))
```
...
* For each dimension, find all pairs of volumes that have equal (min,max) or (max,min) range boundaries
...
The key to understanding the correctness of this approach is that abutting on more than 1 dimension is not possible, since the space has been unoverlapped; So, what of the remaining dimensions? If the hyperrectangles can be fused into a valid hyperrectangle, the other dimension ranges must all be equal! If they weren't the space we're describing is no longer a cartesian product of orthogonal ranges (since two unmatched ranges lie parallel).

In terms of runtime, I am almost certain this algorithm is suboptimal; in the worst case, we could repeat the inner process equal to the number of hypervolumes being fused. There is almost certainly a deeper structure to the problem that is being ignored in doing this. In terms of final solution, I am not sure if this algorithm is optimal. The hierarchical nature (e.g. fuse a,b, then fuse ab,c) of building the solution suggests there are several paths towards the optima. It's unclear if the solution space is "convex", or if we've landed in a local optima. Due to the precise requirements of the fusings being rectangular, I have a gut feeling & no mathematical justification that the solution space is in fact convex. 

```{r}
fuse_abutted_hyperrectangles <- function(volumes) {
  ...
  while (fuses_possible) {
    ...
    candidate_fuses <- fused_volumes %>%
      inner_join(volumes, by = c('dimension' = 'dimension',
                                 'max' = 'min'),
                 suffix = c('.left', '.right')) %>%
...
```{r}
unoverlapped_hyperrectangles <- unoverlapped_hyperrectangles %>%
  mutate(volume_id = new_volume_id) %>%
  select(-new_volume_id)
fused_unoverlapped_hyperrectangles <- fuse_abutted_hyperrectangles(unoverlapped_hyperrectangles)
plot_rectangles(fused_unoverlapped_hyperrectangles)
```

And just like that, we've gone from 15 to 7 volumes! As stated earlier, the optimality of this approach is not certain.
