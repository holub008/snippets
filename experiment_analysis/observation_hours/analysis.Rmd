...
TODO Moran's I for assessment of hours similarity

### Program Density Estimation

Are required number of hours tied to geographic program density? One hypothesis is that increased program density correlates with descreased availability of student openings.
...
points$dw <-predict(density_estimate, points)

ggplot(points) +
  geom_raster(aes(x = Longitude, y = Latitude, fill = density_estimates), interpolate = TRUE) +
  geom_polygon(data = us, aes(x=long, y = lat, group = group), fill=NA, color='black') + 
  scale_fill_distiller(palette=1, direction=1) + 
  guides(fill=guide_colorbar(title='Required Hours',
                             barwidth = 2, barheight = 20, 
                             ...
                             label.theme = element_text(size=20),
                             draw.ulim = FALSE, draw.llim = TRUE)) +
  ggtitle('PTCAS Program Density (KDE)') +
  theme(plot.title = element_text(size=30),
        axis.text=element_text(size=20),
        axis.title=element_text(size=20),
        axis.line=element_blank(),axis.text.x=element_blank(),
...
```{r}
# using actual county population
library(sp)
library(maps)
library(maptools)
library(noncensus)
...
counties_clean <- counties %>%
  mutate(fips = sprintf("%d%s", as.integer(as.character(state_fips)), as.character(county_fips)))
...
latlon_to_populations <- function(pointsDF) {
    countyCentroids <- map('county', fill=TRUE, col="transparent", plot=FALSE)
    IDs <- sapply(strsplit(countyCentroids$names, ":"), function(x) x[1])
    counties_sp <- map2SpatialPolygons(countyCentroids, IDs=IDs,
    ...
    data.frame(county_name = sapply(counties_sp@polygons, function(x) x@ID)[indices],
                              stringsAsFactors = FALSE) %>%
      left_join(county_fips_clean, by = c('county_name' = 'polyname')) %>%
      left_join(counties_clean, by = c('fips' = 'fips')) %>%
      pull(population)
}

testPoints <- data.frame(x = c(-90, -120), y = c(44, 44))

pops <- latlon_to_populations(oh_data %>% select(Longitude, Latitude))
oh_data$population <- pops

ggplot(oh_data) +
  geom_point(aes(log(population), Minimum.Hours.Imputed))

m <- lm(log(Minimum.Hours.Imputed) ~ Latitude*Longitude + density_estimate + population, oh_data)
summary(m)
```
